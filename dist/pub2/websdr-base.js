...
function mem_show()
{
   ...
   for (i=0;i<memories.length;i++) {
      ...
      m=memories[i].mode;
      s+='<tr>';
      s+='<td><input type="button" title="recall" value="Вызвать" onclick="mem_recall('+i+')"><input type="button" title="erase" value="Удалить" onclick="mem_erase('+i+')"><input type="button" title="store" value="Сохранить" onclick="mem_store('+i+')"></td>';
   ...
   s+='<tr>';
   s+='<td><input type="button" disabled title="recall" value="Вызвать" onclick="mem_recall('+i+')"><input type="button" disabled title="erase" value="Удалить" onclick="mem_erase('+i+')"><input type="button" title="store" value="Сохранить" onclick="mem_store('+i+')"></td>';
   s+='<td>(new)</td>';
   s+='</tr>';
...
// draw the diagram that shows the other listeners
{
   ...
   for (b=0;b<nbands;b++) {
      if (!uu_compactview) {
         s+="<p><div align='left' style='width:1px; background-color:black;'><div class=others>";
...
function document_username()
{
  var x= readCookie('username');
  if (x) {
    document.write('<a id="please">Ваше Имя: ');
